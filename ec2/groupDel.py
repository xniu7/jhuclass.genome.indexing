...
def delete(region,group_names):
    ...
    for depend_group in groups:
        for rule in depend_group.rules:
            for grant in rule.grants:
                depend_group.revoke(ip_protocol=rule.ip_protocol, from_port=rule.from_port, to_port=rule.to_port, src_group=grant)        
    
    #Deleting group
    for target_group in groups:
        conn.delete_security_group(target_group.name)
